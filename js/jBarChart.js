...
(function($){
	if($.jChart == undefined){
		console.error("jBarChart depended on jChart");
		return;
	}	
	var barChart = {
		/**
		...
		/**
		 * render bar chart on the canvas. If jGraphic is available. Then using jGraphic to render the chart for interactive
		 */
		render: function (b, i) {
			...
			if (this._isHorizontalChart()) {
				...
				if (this.options.showDataValues) {
					...
					this._addText(x, y, value, {font:this.options.labelFont},this.bContext);
				}
				if (this.options.showDataLabels) {
					...
					this._addText(x, y, label, { align: align, angle: angle, font: this.options.labelFont }, this.bContext);
				}
			} else {
				if (this.options.showDataValues) {
					var t = this.options.type,
					...
					var effectAdjust = (this.options.addEffect) ? this._getShadowWidth(b.width)/2 : 0,
					    ...
					    y = y0 - 3;
					this._addText(x, y, value, { align: "center", font: this.options.labelFont }, this.bContext);
				}
				if (this.options.showDataLabels) {
					var x = x0 + b.width / 2,
					    y = y0 + b.height+5,
						...
						label = (this._isGroupChart() && this.options.showGroupLabels) ? b.groupLabel : b.label;					
					this._addText(x, y, label, { align: align, angle: angle, vAlign: "top", font: this.options.labelFont }, this.bContext);
				}
			}
		},
			...
			_calculateVBar: function() {
				...
				for (var i = 0; i < d.length; i++) {
					...
					this.chartData.push({
                        ...
                        groupIndex: 0, //always 0 for chart types that do not have groups
                        stackIndex: i //stack index is equivalent to index for non-stack charts
					});
				}
			},
			/*
				...
				for (var i = 0; i < d.length; i++) {
					for (var j = 0; j < d[i].length; j++) {
						var count = i * d[0].length + j + 1,
                            ...
                            stackIndex = ((i * d[0].length) + (j+1)) - 1; //add 1 to account for 0 index, subtract 1 at the end to get real value
						this.chartData.push({
							value: d[i][j],
                            ...
                            groupIndex: i,
                            stackIndex: stackIndex //stack index is equivalent to index for non-stack charts
						});
					}
				}
			},
				...
				for (var i = 0; i < d.length; i++) {
					...
					for (var j = 0; j < d[i].length; j++) {
						...
						this.chartData.push({
							...
							color: this.options.colors[j % this.options.colors.length],
                            groupIndex: i,
                            stackIndex: i //stack index is equivalent to groupIndex for non-grouped, stacked charts
						});
						lastWidth += width;
					}
			...
			*/
			...
			_drawInteractiveBar: function (x, y, width, height, color, shadowWidth, i) {
				var metadata =
                    ...
                    && $.isArray(this.options.metadata)
                    && this.options.metadata.length > i
                        ? this.options.metadata[i]
                        : {};

				var groupIndex = this.chartData[i].groupIndex;
                var groupMetadata =
                    this.options.groupMetadata != undefined
                    && $.isArray(this.options.groupMetadata)
                    && this.options.groupMetadata.length > groupIndex
                        ? this.options.groupMetadata[groupIndex]
                        : {};

                var stackIndex = this.chartData[i].stackIndex;
                var stackMetadata =
                    this.options.stackMetadata != undefined
                        && $.isArray(this.options.stackMetadata)
                        && this.options.stackMetadata.length > stackIndex
                        ? this.options.stackMetadata[stackIndex]
                        : {};

                var bar = (this._isHorizontalChart())
                    ? this.iCanvas.rectangle(x,y,width,height,
                        {fillStyle:color, shadowLayer:{offsetY: shadowWidth, width: shadowWidth, color: "black"},gradientLayer:{isHorizontal: false},
                            metadata: metadata, groupMetadata: groupMetadata, stackMetadata: stackMetadata})
                    : this.iCanvas.rectangle(x,y,width,height,
                        {fillStyle:color, shadowLayer:{offsetX: shadowWidth, width: shadowWidth, color: "black"},gradientLayer:{isHorizontal: true},
                            metadata: metadata, groupMetadata: groupMetadata, stackMetadata: stackMetadata});

                bar.index = i;
                bar.groupIndex = groupIndex;
                bar.stackIndex = stackIndex;
                if (this.interactiveObjects == undefined) this.interactiveObjects = [];
				this.interactiveObjects[i] = bar;
			...
			_getShadowWidth:function(width) {
				var tmp = Math.floor((width - this.options.barChart.shadow) / 4);
				var shadowWidth = this.options.shadow < tmp ? this.options.shadow : tmp;
				return shadowWidth;
			},
			
			_applyGradient: function(x, y, width, height, color) {
